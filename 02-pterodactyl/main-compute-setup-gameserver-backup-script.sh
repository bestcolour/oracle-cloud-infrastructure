...
sudo docker run --rm \
  ...
  --volume /:/hostfs:ro \
  rclone/rclone sync /hostfs/srv/pterodactyl cloud_backup:/pterodactyl_backups/panel_data --progress

sudo docker run --rm \
  --volume /etc/rclone:/config/rclone \
  --volume /:/hostfs:ro \
  rclone/rclone sync /hostfs/etc/pterodactyl cloud_backup:/pterodactyl_backups/wings_config --progress

# /var/lib/pterodactyl contains the Wings game server volumes and local backup archives
sudo docker run --rm \
  --volume /etc/rclone:/config/rclone \
  --volume /:/hostfs:ro \
  rclone/rclone sync /hostfs/var/lib/pterodactyl cloud_backup:/pterodactyl_backups/game_servers --progress
